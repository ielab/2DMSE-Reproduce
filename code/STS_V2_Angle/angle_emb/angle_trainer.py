...
parser.add_argument('--layer_num_cut', type=int, default=None)
...
parser.add_argument("--embedding_size", type=int, default=768,
                    help="Specify embedding size, default 768")
...
# configure MRL
parser.add_argument('--apply_mrl', type=int, default=0, choices=[0, 1],)
...
def main():
    ...
    if args.layer_num_cut is not None:
        model.backbone.encoder.layer = model.backbone.encoder.layer[:args.layer_num_cut]
        #also change config so when saving, it will save the correct number of layers
        model.backbone.config.num_hidden_layers = args.layer_num_cut
    ...
    if args.apply_ese or args.apply_mrl:
